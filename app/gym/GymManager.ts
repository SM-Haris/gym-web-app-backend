...
class GymManager {
  ...
  static async getGymRevenue(req: UserRequest) {
    try {
      ...
      const members = await MemberHandler.getMembersByDate(req.params.gym_id,toDateObj)
      ...
      for (let i = 1; i <= daysDifference; i++) {
        ...
        while (memberIndex < members.length && members[memberIndex].created_at.toISOString().split('T')[0] <= formattedDate && formattedDate <= toDateObj.toISOString().split('T')[0]) {
          revenueSum = revenueSum + members[memberIndex].fee;
          memberIndex++;
        }
